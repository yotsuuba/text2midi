    ...
    def create_midi(self, text):
        ...
        for line in lines:
            ...
            if self.is_cluster(line.strip()):
                ...
                # Adjust label timing to account for rests
                ...
                for word in words:
                    ...
                    for i, char in enumerate(chars):
                        ...
                        if (self.treat_underscore_as_rest and 
                            i < len(chars) - 1 and chars[i + 1] == '_'):
                            current_time += self.short_silence_duration
        ...
        # Ensure minimum 2-second silence at the end
        final_silence_needed = max(self.final_silence, 2.0)  # At least 2 seconds
        current_time += final_silence_needed
        
        # Adjust the last label's end time if needed
        if labels:
            last_label = labels[-1]
            last_label['end'] = min(last_label['end'], current_time - final_silence_needed)
